...
//example custom data RS(-angle60-dir-1) //this wuld meen that the rotor wuld stay at a 60degree angle with direction -1 the third is of no importance
...
{
lcdWord = "jifrs"; //the one i void log must match this one
...
try{
wCDVal = wCD.Substring(wCD.IndexOf("(")+2);
}catch{};
try{
wCDTyp = wCD.Substring(0,wCD.IndexOf("("));
}catch{};
try{
anglTrgt=float.Parse(wCD.Substring(wCD.IndexOf("-angle")+6,wCD.IndexOf(")",wCD.IndexOf("-angle")+6)-wCD.IndexOf("-angle")-6));
}catch{};
try{
anglTrgt=float.Parse(wCD.Substring(wCD.IndexOf("-angle")+6,wCD.IndexOf("-",wCD.IndexOf("-angle")+6)-wCD.IndexOf("-angle")-6));
}catch{};
try{
direc=float.Parse(wCD.Substring(wCD.IndexOf("-dir")+4,wCD.IndexOf(")",wCD.IndexOf("-dir")+4)-wCD.IndexOf("-dir")-4));
}catch{};
try{
direc=float.Parse(wCD.Substring(wCD.IndexOf("-dir")+4,wCD.IndexOf("-",wCD.IndexOf("-dir")+4)-wCD.IndexOf("-dir")-4));
}catch{};
try{
invRef=wCD.Substring(wCD.IndexOf("-invref")+7,wCD.IndexOf(")",wCD.IndexOf("-invref")+7)-wCD.IndexOf("-invref")-7);
}catch{};
try{
invRef=wCD.Substring(wCD.IndexOf("-invref")+7,wCD.IndexOf("-",wCD.IndexOf("-invref")+7)-wCD.IndexOf("-invref")-7);
}catch{};
...
try{
unFlip=wCD.Substring(wCD.IndexOf("-unflip")+7,wCD.IndexOf(")",wCD.IndexOf("-unflip")+7)-wCD.IndexOf("-unflip")-7);
}catch{};
try{
unFlip=wCD.Substring(wCD.IndexOf("-unflip")+7,wCD.IndexOf("-",wCD.IndexOf("-unflip")+7)-wCD.IndexOf("-unflip")-7);
}catch{};
...
if(REF != null){
if(invRef=="true"){
anglTrgt = anglTrgt-Convert.ToSingle(REF.Angle*180/Math.PI);
}else{
anglTrgt = anglTrgt+Convert.ToSingle(REF.Angle*180/Math.PI);
};

if(0>anglTrgt){
anglTrgt = 360+anglTrgt;
};
if(anglTrgt>360){
anglTrgt = 0+(anglTrgt-360);
}; //invRef
}; //REF
...
if(wCDTyp=="RS"&&wCD.IndexOf("-ref") != -1){
REF = wheel;
};
if(wCDTyp=="RS"&&wCD.IndexOf("-ref") == -1){
