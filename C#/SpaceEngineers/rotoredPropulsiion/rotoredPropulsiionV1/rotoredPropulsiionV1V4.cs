...
IMyMotorSuspension wheel = wheels[whilWheel];
...
float custRol=0;
try{
custMov=float.Parse(wCD.Substring(0,wCD.IndexOf("|")));
custRot=float.Parse(wCD.Substring(wCD.IndexOf("|")+1,((wCD.IndexOf("|",wCD.IndexOf("|")+1))-wCD.IndexOf("|"))-1));
custRol=float.Parse(wCD.Substring(wCD.IndexOf("|",wCD.IndexOf("|")+1)+1));
}catch{};

log(consoles,"wCD: "+wCD);
...
wheel.SetValue<float>("SteerSpeed",(mov.X*custRol));
