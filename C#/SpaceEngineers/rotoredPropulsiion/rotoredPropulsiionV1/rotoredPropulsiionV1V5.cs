...
int RPM=10; //howe mutch rpm you want
...
if(wCDTyp=="drive"){
wheel.TargetVelocityRPM=RPM*(mov.Z*custRot);
};

if(wCDTyp=="steer"){
wheel.TargetVelocityRPM=SRPM*(mov.X*(custMov*custRol));
if(mov.X==0){
var angl = wheel.Angle;
if(angl>(SRPM/2) && 180>angl){
wheel.TargetVelocityRPM=SRPM*(1);
}; //-angle
if(angl>180 && (360-(SRPM/2))>angl){
wheel.TargetVelocityRPM=SRPM*(-1);
}; //+angle
};
};
