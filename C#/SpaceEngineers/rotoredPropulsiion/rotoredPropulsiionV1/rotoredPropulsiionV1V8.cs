...
int controllType = 2; // |1: Turn|2: Tank|3: ?| 
...
int steering=30; //max steering angle
//options end
//****//****//****//****//****//****//****//****//****//****//****//****//****//****//****//
//variables
int runCount = 0;
...
if(custMov>0){
float angLow = Convert.ToSingle((0+steering)*Math.PI/180); //180
float angHigh = Convert.ToSingle((360-steering)*Math.PI/180); //180
if(angl>angLow && angHigh>angl){wheel.TargetVelocityRPM=0;}else{
wheel.TargetVelocityRPM=SRPM*(mov.X*(custMov*custRol)*custRot);
};
};
if(0>custMov){
float angLow = Convert.ToSingle((0+steering)*Math.PI/180); //180
...
if(mov.X==0){
float angLow = Convert.ToSingle((SRPM/7.5*runSpeed)*Math.PI/180); //ca 0
float angHigh = Convert.ToSingle((360-(SRPM/7.5*runSpeed))*Math.PI/180); //ca 360

if(angl>angLow && angMid>angl){
wheel.TargetVelocityRPM=SRPM*(-1);
}; //-angle
if(angl>angMid && angHigh>angl){
wheel.TargetVelocityRPM=SRPM*(1);
}; //+angle
};
};
